...
import MasterKeyDialog from "./master-key-dialog";
import AuthenticatorGrid from "./authenticator-grid";
import AddAuthenticatorButton from "./add-authenticator";

export default function Main() {
  const { setMasterKey } = useAppContext();
  ...
  return (
    <>
      <MasterKeyDialog onAuthenticated={handleAuthentication} />
      {isAuthenticated && (
        <>
          <SettingsButton />
          <main className="mx-auto p-4 space-y-6 max-w-dvw overflow-x-hidden">
            <div className="rounded-4xl border-2 border-lime-400 flex flex-col md:items-center justify-center bg-gradient-to-b from-lime-500 to-background h-96 gap-5 md:text-center text-left px-10">
              ...
              <p className="opacity-50 md:text-lg tracking-tight max-w-xl">
                A privacy-focused authenticator manager that stores all data
                locally in your browser. Free and open source.
              </p>
            </div>
